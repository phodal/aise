...
未来几十年，将会出现人机混合编写代码的模式。在这种背景下，很容易将代码编写速度误认为是生产力。为了在最大限度减少风险的同时，充分利用AI辅助编码的优势，我们需要：

1. **设定现实的期望**：AI辅助编程可以帮助完成特定任务，但由于其一致性和正确性不足，无法替代人类程序员。承认这些限制，关注增强而不是替代。确保所有AI生成的代码都经过测试覆盖。
2. **将代码质量设为关键绩效指标（KPI）**：对人类和机器都要执行最低标准。在交付流程中实施自动化质量检查。使用可靠且经过验证的指标，最大限度地提高信号并减少误报。
3. **进行持续的代码检查**：AI生成的代码需要被理解。绝不接受团队无法掌握或未审查的代码。可视化整个团队的代码熟悉度，以便及时发现任何潜在的知识孤岛。
4. **认识到理解代码的重要性日益提升，而不仅仅是编写代码**：理解AI生成的代码将成为一项关键技能，因此确保你的流程、实践和培训适应这一变化。

正如我们所见，这里有一个共同的主题。要在AI辅助编码中取得成功，我们需要让有技能的人类参与，同时引入专门的工具和流程以确保代码库的健康。
导航这一新领域意味着我们需要重新聚焦于代码质量和持续学习。这是至关重要的。
