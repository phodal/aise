...
3. 确保人机协作：设计目标时应考虑到人机协作的需求。AI 的输出需要与开发人员的工作流程紧密结合，并允许开发人员对 AI 生成的结果进行快速审查和修改。
4. 制定数据安全与隐私策略：根据组织的规模和敏感度，制定清晰的数据安全与隐私保护策略。对于中大型组织，特别是在数据敏感度高的情况下，自建 AI4SE 体系可能更为合适。
5. 定期评估与反馈机制：建立定期评估机制，收集用户的反馈，并根据实际使用情况调整 AI4SE 目标和策略。这样可以确保 AI 技术能够随着业务需求的变化不断优化和改进。
